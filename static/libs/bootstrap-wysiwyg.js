  ...
  $.fn.wysiwyg = function (userOptions) {
    ...
    // Adding Toggle HTML based on the work by @jd0000, but cleaned up a little to work in this context.
      ...
      insertFiles = function (files) {
        ...
        $.each(files, function (idx, fileInfo) {
          if (/^image\//.test(fileInfo.type)) {
            ...
            uploadFile(fileInfo)
              .done(function (resp) {
                var url = options.responseHandler(resp);

                if (!url) {
                  return options.fileUploadError("file-reader", resp);
                }
      ...
      uploadFile = function (file) {
        ...
        formData.append(options.alias, file);
      ...
      bindToolbar = function (toolbar, options) {
        ...
        toolbar.find('[data-edit="blockquote"]').on('click',function(){
          insertBlockquote();
        });
